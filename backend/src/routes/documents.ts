...
import { assetScope, permit } from "../rbac.js";
...
documentsRouter.post(
  ...
  async (req: AuthRequest, res) => {
    ...
    if (!req.file)
      throw new AppError(400, "Choose an allowed file up to 10 MB");
    const type = String(req.body.type || "OTHER").slice(0, 50);
    ...
    const updated = await prisma.assetDocument.update({
      where: { id: row.id },
      data: { url: `/api/documents/${row.id}/download` },
    });
    await audit(req, "UPLOAD", "AssetDocument", row.id, undefined, {
      name: row.name,
      type: row.type,
      size: row.size,
    });
...
documentsRouter.get("/:id/download", async (req: AuthRequest, res) => {
  const row = await prisma.assetDocument.findFirst({
    where: {
      ...
      organizationId: req.user!.organizationId,
      asset: assetScope(req),
    },
  });
  if (!row || !row.storageKey) throw new AppError(404, "Document not found");
  res.download(path.join(uploadDir, path.basename(row.storageKey)), row.name);
});
documentsRouter.delete(
  ...
  async (req: AuthRequest, res) => {
    const id = String(req.params.id),
      row = await prisma.assetDocument.findFirst({
        where: { id, organizationId: req.user!.organizationId },
      });
    if (!row) throw new AppError(404, "Document not found");
    await prisma.assetDocument.delete({ where: { id } });
    if (row.storageKey)
      unlink(path.join(uploadDir, path.basename(row.storageKey)), () => {});
    await audit(req, "DELETE", "AssetDocument", id, {
      name: row.name,
      type: row.type,
    });
    res.status(204).end();
